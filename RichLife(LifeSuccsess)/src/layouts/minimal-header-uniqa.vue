   ...
   <header :class="{'whtieHeader' : token != undefined}">
      <i-container :align="'center'" :display="'flex'" :justify="'space-between'">
         ...
         <div class="user-details" v-if="token != undefined">
            ...
            <user-menu @getInTouch="$emit('getInTouch')" @click="onShowConfirmLogout" @close="closeUserMenu" @getMoney="$emit('getMoney')" :showUserMenu="showUserMenu"></user-menu>
         </div>
      </i-container>

      <!-- mobile-menu -->
      <mobile-menu-uniqa @openNote="$emit('openNote')" @logout="onShowConfirmLogout" @showJoinModal="onShowJoin" @showGetInTouch="onShowGetInTouch" @changeShow="onChangeShow" @callUs="$emit('callUs')" :show="showMenu" @close="onCloseMenu" />
...
@Component
export default class MinimalHeaderUniqaComponent extends Vue {
   ...
   mounted() {
      ...
      // window.addEventListener('scroll', this.scrollHeaderStatic)
   }
