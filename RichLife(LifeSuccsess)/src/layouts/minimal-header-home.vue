   ...
   <header :class="{'whtieHeader' : token != undefined}">
      <i-container :align="'center'" :display="'flex'" :justify="'space-between'">
         ...
         <div class="i-button-group" v-if="token == undefined">
            ...
            <router-link to="register-referal" class="i-button-link small green" >Регистрация</router-link>
      ...
      <!-- mobile-menu -->
      <mobile-menu-home :user="user" @openNote="$emit('openNote')" @logout="onShowConfirmLogout" @showJoinModal="onShowJoin" @showGetInTouch="onShowGetInTouch" @changeShow="onChangeShow" @callUs="$emit('callUs')" :show="showMenu" @close="onCloseMenu" />
...
@Component
export default class MinimalHeaderHomeComponent extends Vue {
